...
/**
 * HubCore – ClassicPvPServerSelector.php
 *
	...
	public function updateName() {
		...
		if($server->isOnline()) {
			if($server->isAvailable()) {
				$this->setCustomName(ChatUtil::centerPrecise(Utils::translateColors("&l&6ClassicPvP &a(&e{$server->getOnlinePlayers()}&a/&e{$server->getMaxPlayers()}&a)&r\n\n&l&aClick to connect!"), null));
			} else {
				$this->setCustomName(ChatUtil::centerPrecise(Utils::translateColors("&l&6ClassicPvP &6(&e{$server->getOnlinePlayers()}&6/&e{$server->getMaxPlayers()}&6)&r\n\n&l&cUnavailable"), null));
			}
		} else {
			$this->setCustomName(ChatUtil::centerPrecise(Utils::translateColors("&l&6ClassicPvP &6(&coffline&6)&r\n\n&l&cUnavailable"), null));
		}
	}

	public function getNetworkServer() {
		return Main::getInstance()->getNetworkManager()->getNodes()["CPVP"]->getServers()[$this->serverId];
	}
